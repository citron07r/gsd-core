...
function selectRuntimesFromArgs(runtimeArgs) {
  if (runtimeArgs.includes('--all')) {
    return ['claude', 'kimi', 'kilo', 'opencode', 'codex', 'copilot', 'antigravity', 'cursor', 'windsurf', 'augment', 'trae', 'qwen', 'hermes', 'codebuddy', 'cline', 'zcode', 'droid'];
  }
...
function install(isGlobal, runtime = DEFAULT_RUNTIME, options = {}) {
  ...
  const _DESCRIPTOR_AGENTS_RUNTIMES = new Set(['cursor', 'windsurf', 'augment', 'trae', 'codebuddy', 'copilot', 'antigravity', 'droid']);
...
// 'droid' is intentionally absent from the interactive allRuntimes menu but is
// fully supported via the --droid flag (e.g. `node bin/install.js --droid
// --global`). The descriptor-driven pipeline handles droid natively:
