...
function selectRuntimesFromArgs(runtimeArgs) {
  if (runtimeArgs.includes('--all')) {
    return ['claude', 'kimi', 'kilo', 'opencode', 'codex', 'copilot', 'antigravity', 'cursor', 'windsurf', 'augment', 'trae', 'qwen', 'hermes', 'codebuddy', 'cline', 'zcode', 'droid', 'pi'];
  }
  ...
  if (runtimeArgs.includes('--droid')) selected.push('droid');
...
function install(isGlobal, runtime = DEFAULT_RUNTIME, options = {}) {
  ...
  // droid: agents are emitted into droids/ via the descriptor layout's
  // convertClaudeAgentToDroidAgent converter.
  // pi: the descriptor layout deliberately declares NO agents kind (Pi has no
  // subagents by design) — membership here keeps the inline loop off so no
  // agents/ dir is fabricated.
  const _DESCRIPTOR_AGENTS_RUNTIMES = new Set(['cursor', 'windsurf', 'augment', 'trae', 'codebuddy', 'copilot', 'antigravity', 'droid', 'pi']);
  ...
  if (isKimi) {
    ...
    // via its own prune pass. No further action needed. Runtimes whose layout
    // declares no agents kind (e.g. pi) correctly install none.
    const _agentsCap = _capabilityRegistry && _capabilityRegistry.runtimes && _capabilityRegistry.runtimes[runtime];
    const _agentsLayout = _agentsCap && _agentsCap.runtime && _agentsCap.runtime.artifactLayout;
    const _agentsScope = _agentsLayout ? (isGlobal ? _agentsLayout.global : _agentsLayout.local) : null;
    const _hasAgentsKind = Array.isArray(_agentsScope) && _agentsScope.some((e) => e && e.kind === 'agents');
    console.log(_hasAgentsKind
      ? `  ${dim}↳${reset} Agents installed via descriptor-driven layout (${runtime})`
      : `  ${dim}↳${reset} No agents surface for ${runtime} (descriptor declares none)`);
...
// 'droid' and 'pi' are intentionally absent from the interactive allRuntimes
// menu but fully supported via their flags (e.g. `node bin/install.js --droid
// --global`, `--pi --global`). The descriptor-driven pipeline handles droid natively:
